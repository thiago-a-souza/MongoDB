...
- ***$push*** appends an item to an array
  ...
  - ***$slice*** limits the resulting array with the initial/final items using a positive/negative argument and requires the *$each* modifier. A zero argument removes all array items. Unlike the *$slice* used in projection, it only accepts an integer as argument.
...
- **Positional $ operator** identifies an element in the array without specifying the index; the array field name must appear in the query. If there are multiple matches, only the first occurrence will be updated. Field must be enclosed in quotes otherwise it throws an error.
