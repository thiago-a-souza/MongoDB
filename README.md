...
At a first glance, it may seem odd that NoSQL databases are *eventually consistent*. However, for some applications having the most recent data is not critical. For example, it's not really important that a post is not displayed immediately to all followers on a social network; it's ok that eventually they will be consistent. In fact, some NoSQL databases, like Cassandra, provide a *tuneable consistency*, which means that the level of consistency required is configured.
