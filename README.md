...
MongoDB is continuously  evolving to incorporate new features. Initially, transactions were supported only at the document level, in other words, ACID properties were guaranteed only within fields of the same document. Starting in version 4.0, it also supports multi-document transactions at a performance cost. In terms of durability, before version 2.0 journaling was not enabled by default, so it could not ensure that changes were permanent after a server crash. 
